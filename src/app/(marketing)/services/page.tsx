      ...
      <Section padding="lg">
        <div className="space-y-12">
          {services.map((service) => (
            <article
              key={service.slug}
              className="rounded-3xl border border-[var(--color-border)] bg-[var(--color-service-surface)] p-10 shadow-[0_35px_90px_-45px_rgba(37,99,235,0.55)] backdrop-blur"
            >
                    ...
                    <div className="mt-4 grid gap-3 text-sm text-[var(--color-muted-foreground)]">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {service.stack.map((tool) => (
                          <span
                            key={tool}
                            className="rounded-full bg-[var(--color-muted)]/70 px-3 py-1 text-xs font-medium uppercase tracking-[0.3em]"
                          >
                            {tool}
                          </span>
                        ))}
                      ...
                      <div className="mt-4 grid gap-3 sm:grid-cols-2">
                        {service.metrics.map((metric) => (
                          <div
                            key={metric.label}
                            className="rounded-2xl bg-[var(--color-muted)]/80 px-4 py-3 text-sm shadow-inner shadow-blue-500/5"
                          >
